...
public class MauiYoutubeService
{
        ...
        public MauiYoutubeService(IConfiguration config, ILogger<MauiYoutubeService> logger, Mp3LibraryMaui library)
        {
            this.config = config;
            this.logger = logger;
        ...
        private readonly ILogger<MauiYoutubeService> logger;
        ...
        public async Task CacheVideoAsync(string id, string title, IProgress<double> progress)
        {
            ...
            var destinationPath = Path.Combine(config[ConfigKeysMaui.MediaFolder]!, "YouTube Cache");
            if (!Directory.Exists(destinationPath))
                Directory.CreateDirectory(destinationPath);
            destinationPath = Path.Combine(destinationPath, fileName);
            var ffmpegLocation = config[ConfigKeysMaui.FFMpegLocation] ?? throw new Exception("Missing ffmeg path in config: " + ConfigKeysMaui.FFMpegLocation);
